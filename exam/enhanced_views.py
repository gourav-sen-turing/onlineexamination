...
from .models_enhanced import (
    ...
    NumericalQuestion, Result, QuestionType
)
from .forms_enhanced import (
    ...
    NumericalQuestionForm, DynamicQuestionAnswerForm,
        ...
        for row in reader:
            ...
            if question_type == 'MCQ':
                ...
                TrueFalseQuestion.objects.create(
                    course=course,
                    question_text=row['question'],
                    correct_answer=row.get('correct_answer', '').lower() == 'true',
                    marks=int(row.get('marks', 1)),
                    explanation=row.get('explanation', ''),
                    difficulty_level=int(row.get('difficulty', 2))
                )
...
def take_exam_view(request, course_id):
    ...
    return render(request, 'exam/enhanced/student_take_exam.html', context)
